...
export class ImageService {
  ...
  public getOneImage(image_id:string):Observable<Image>{
    let data = this.http.get<Image>(this.env.imagesUrl+"/"+image_id)
    .pipe(catchError(this.handleError));
    return (data);
  }

  public deleteImage(image_id:string, body:{}):Observable<Image>{
    let userUrl = this.env.imagesUrl+"/"+image_id;
    let data= this.http.delete<Image>(userUrl,body)
    .pipe(catchError(this.handleError));
    return(data); 
  }
